...
class _NowPlayingScreenState extends State<NowPlayingScreen>
  ...
  Duration _parseDuration(String duration) {
    final parts = duration.replaceAll('.', ':').split(':');
  ...
  void initState() {
    ...
    //----------------------------------
    _musicListener = () {
      final track = MusicPlayerService.instance.currentTrack;

      if (track == null) {
        return;
      }

      if (track.videoId == _currentVideoId) {
        return;
      }

      debugPrint('🔄 TRACK CHANGED = ${track.title}');
      ...
      setState(() {
        ...
        _currentImageUrl = track.thumbnail;
    ...
    MusicPlayerService.instance.addListener(_musicListener!);
  ...
  Future<void> _loadAndPlay() async {
    try {
      final playerData = await playerRepository.getPlayer(_currentVideoId);

      final player = MusicPlayerService.instance.player;

      await player.setAudioSource(
        AudioSource.uri(Uri.parse(playerData.streamUrl)),
      );

      await player.load();

      debugPrint('REAL DURATION = ${player.duration}');
      ...
      if (mounted) {
        ...
        if (e.toString().contains('age-restricted')) {
          errorMessage =
              'Lagu ini dibatasi oleh usia. Silakan pilih lagu lain.';
  ...
  @override
  void dispose() {
    debugPrint('❌ NOW PLAYING DISPOSE');

    if (_musicListener != null) {
      MusicPlayerService.instance.removeListener(_musicListener!);
    }
